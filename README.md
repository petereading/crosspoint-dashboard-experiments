...
- **Weather** — current conditions, today's sunrise and sunset, and a five-day forecast from the same Worker, using Open-Meteo.
- **Moon** — today's moon phase and the next four key phases.
- **RSS** — the latest headlines from a public RSS or Atom feed.
- **Today** — notable events from Wikipedia's On This Day data.
- **Quote** — Wikiquote's quote of the day.
- **Bitcoin** — the current BTC price, 24-hour movement, and a seven-day chart in GBP, USD, or EUR.
- **Solar System** — an approximate current heliocentric chart of the eight planets.
- **Custom Image** — downloads any supported BMP from a complete public HTTPS URL, such as another Worker or a photo.

Enter one **Dashboard Worker URL** in the web settings, without a `.bmp` route. The firmware adds the selected route,
`device=x3|x4`, and `orientation=portrait|landscape` automatically, so every generated card shares one deployment.
RSS also needs a public feed URL and optionally a short title; Today has a language code; Bitcoin has a currency
setting. Custom Image has a separate complete URL field. The Worker, RSS feed, and Custom Image URLs can also be
entered on the device by opening the corresponding Lock Screens item.

Each card has its own refresh interval under Settings → System (defaults: Clock 1, Weather 15, Moon 240, RSS 30,
Today 240, Quote 240, Bitcoin 15, Solar System 60, and Custom Image 30 minutes). Downloads use a temporary file,
validate the BMP, and only then replace that card's cache. A failed refresh keeps the last successful image; when no
valid cache exists, unattended lock-screen mode displays the normal CrossPoint sleeping screen and retries on the
next interval.

**Orientation** — all Worker-generated cards use **landscape** (the default) or **portrait**, controlled by Settings →
System → Lock Screen Orientation. Custom Image follows the downloaded BMP's aspect ratio and is proportionally scaled
down and centred when necessary.
...
The paste-ready [Cloudflare dashboard Worker](./examples/cloudflare-dashboard-worker.js) generates all dashboard cards
as one-bit BMPs from one deployment:
...
- `/bitcoin.bmp?currency=GBP&tz=Europe/London`
- `/solar.bmp?tz=Europe/London`

Every card is selectable directly under **Lock Screens**. RSS accepts public HTTPS RSS or Atom feeds; the firmware URL-
encodes the configured feed automatically. Moon uses the U.S. Naval Observatory for the next four key phases and falls
back to an approximate synodic-cycle calculation when that service is unavailable. Wikipedia Today uses Wikimedia's
current On This Day feed; Wikimedia has announced gradual deprecation of this feed family, so that route may need to be
updated when a replacement is published. Quote uses the English Wikiquote daily page, Bitcoin uses Coinbase Exchange
public market data, and Solar System uses JPL's approximate orbital elements. The compact built-in bitmap font is
Latin/ASCII only, so choose English or another feed/language that can be represented by that character set.
...
backward compatibility by defaulting to X3. The firmware supplies both parameters automatically for every generated
card; include them explicitly in Custom Image URLs when needed.
